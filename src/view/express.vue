      ...
      postPay(this.from)
        ...
        .then(star => {
          ...
          weiXinConfig({ url: window.location.href }).then(conf => {
            ...
            this.from.addressid = '' // 送货地址
            this.from.postStationid = '' // 收货地址
            this.from.code = '' // 取件信息
            this.from.pics = [] // 图片
            this.from.content = '' // 订单备注
            this.from.xiaofei = '' // 小费
            this.from.danliang = '' // 单量
            this.from.size = '' // 规格
            this.addressName = '' // 发货地址名字
            this.quHuoShowId = '' // 取货地址名字
            this.fileIds = [] // 图片上传暂存数组
            setTimeout(function() {
              payLoading.clear()
              that.$router.push({ path: '/kdRecording' })
            }, 8000)
          })
        })
        .catch(err => {
