<template>
  <div class="carBox">
    <van-row class="shoppingItem" v-for="(item,index) in shoppingList" :key="item.id">
      <van-col span="2" @change="listenCheck">
        <div style="margin: 33px auto auto 5px" >
          <van-checkbox checked-color="#f00" v-model="shoppingList[index].isChecked"></van-checkbox>
        </div>
      </van-col>
    ...
    <div class="bottomMain" style="width: 100%">
      <van-row>
        <van-col span="6">
          <van-checkbox v-model="checkedAll" checked-color="#f00" @native.click="allChong" style="margin-top:24px;margin-left:20px">全选</van-checkbox>
        </van-col>
        <van-col span="18" style="display: flex;justify-content: flex-end;margin-top: 13px;">
          <button class="bottombtn2" @native.click="delGoods" style="margin-left:10px">清空购物车</button>
          <button class="bottombtn1" @native.click="buyGoods" style="margin-right: 20px;">￥{{sum}}&nbsp;结算</button>
        </van-col>
      </van-row>
    </div>
